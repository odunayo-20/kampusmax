...
export function CustomersPagination({ page, totalPages, total, pageSize, onPageChange }: CustomersPaginationProps) {
  ...
  return (
    <div className="mt-4 flex flex-col gap-3 border-t border-kampmax-border pt-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-kampmax-text-secondary">
        Showing {start.toLocaleString("en-NG")}–{end.toLocaleString("en-NG")} of {total.toLocaleString("en-NG")} customers
      </p>
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, i) =>
          p === "ellipsis" ? (
            ...
            <Button key={p} variant={p === page ? "primary" : "outline"} size="sm" onClick={() => onPageChange(p)} aria-label={`Page ${p}`} aria-current={p === page ? "page" : undefined} className="min-w-[36px]">
              {p}
            </Button>
          )
        )}
