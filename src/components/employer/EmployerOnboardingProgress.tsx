...
import {
  EMPLOYER_ONBOARDING_STEP_LABELS,
  EMPLOYER_ONBOARDING_STEP_DESCRIPTIONS,
  type EmployerOnboardingStepId,
  type EmployerOnboardingStatus,
} from "@/types/employer";

interface EmployerOnboardingProgressProps {
...
const STEP_GROUPS: { label: string; steps: EmployerOnboardingStepId[] }[] = [
  { label: "Your Profile", steps: [1, 2] },
  { label: "Contact", steps: [3] },
  { label: "Hiring", steps: [4] },
  { label: "Review", steps: [5] },
];

const BLOCKING_STATUSES: EmployerOnboardingStatus[] = ["PENDING_REVIEW", "APPROVED", "REJECTED", "SUSPENDED"];

export function EmployerOnboardingProgress({
  currentStep,
  status,
  completedSteps = [],
  className,
}: EmployerOnboardingProgressProps) {
  const isBlocking = status && BLOCKING_STATUSES.includes(status as EmployerOnboardingStatus);
          ...
          <div>
            ...
            <p className="text-sm text-kampmax-text-secondary">
              {status === "PENDING_REVIEW" &&
                "Kampmax is reviewing your employer profile. You'll be notified once a decision is made."}
              {status === "APPROVED" && "Congratulations! You're ready to hire freelancers."}
              {status === "REJECTED" && "Your profile was not approved. Check your email for details."}
              {status === "SUSPENDED" && "Your employer profile has been suspended."}
            </p>
          </div>
        </div>
      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={cn("text-sm font-medium truncate", isFuture && "text-neutral-400")}>
                      {EMPLOYER_ONBOARDING_STEP_LABELS[step]}
                    </p>
                    <p className={cn("text-[11px] truncate", isFuture && "text-neutral-400")}>
                      {EMPLOYER_ONBOARDING_STEP_DESCRIPTIONS[step]}
                    </p>
                  </div>
...
export function EmployerStepIndicator({
  currentStep,
  completedSteps = [],
}: {
  currentStep: EmployerOnboardingStepId;
  ...
  return (
    <div
      className="hidden md:flex items-center gap-1 overflow-x-auto pb-2 px-2 -mx-2"
      role="navigation"
