...
/**
 * Auth-aware actions on the public employer profile.
 * - Owners (authenticated user id === profile owner) get an "Edit profile"
 *   link to their own private dashboard.
 ...
 */
export function EmployerProfileActions({
  employerUserId,
}: {
  ...
  const ownProfile = status === "authenticated" && user?.id === employerUserId;
  ...
  function handleMessage() {
    ...
    setBusy(true);
    setError(null);
    openConversation(employerUserId)
      .then((conversationId) => {
        if (conversationId) {
          router.push(`/chat/${conversationId}`);
        } else {
          setError("We couldn't open a conversation with this employer.");
        }
      })
