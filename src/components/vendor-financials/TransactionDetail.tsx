...
interface TransactionDetailProps {
  transaction: VendorFinancialTransaction;
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm text-kampmax-text-secondary">{txTypeLabel(transaction.type)}</p>
          <h1 className="text-xl font-bold text-kampmax-text">{transaction.description}</h1>
        </div>
        <div className="flex items-center gap-2">
          <StatusBadge variant={txStatusVariant(transaction.status)} label={txStatusLabel(transaction.status)} />
          {transaction.payoutId && (
            <StatusBadge variant="info" label="Payout" dot={false} className="ml-2" />
          )}
        </div>
      </div>

      {/* Immutable notice */}
      <div className="rounded-lg bg-neutral-50 p-4 border border-kampmax-border">
        <div className="flex items-start gap-3">
          <AlertCircle className="h-5 w-5 text-kampmax-text-secondary mt-0.5 shrink-0" />
          <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          icon={signIcon(transaction.sign) === "arrow-up" ? <ArrowUp className="h-5 w-5 text-kampmax-success" /> : <ArrowDown className="h-5 w-5 text-kampmax-error" />}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-xl border border-kampmax-border bg-white p-4" aria-labelledby="details-heading">
          ...
          <dl className="mt-4 space-y-4">
            <DetailRow label="Transaction ID" value={transaction.id} copyable onCopy={() => copyToClipboard(transaction.id, "Transaction ID")} />
            <DetailRow label="Type" value={txTypeLabel(transaction.type)} />
            <DetailRow label="Status" value={<StatusBadge variant={txStatusVariant(transaction.status)} label={txStatusLabel(transaction.status)} />} />
            <DetailRow label="Direction" value={<span className="inline-flex items-center gap-1">{signIcon(transaction.sign) === "arrow-up" ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}{SIGN_LABELS[transaction.sign]}</span>} />
            ...
            {transaction.payoutId && (
              <DetailRow label="Payout ID" value={<span className="font-mono">{transaction.payoutId}</span>} copyable onCopy={() => copyToClipboard(transaction.payoutId!, "Payout ID")} />
            )}
          </dl>
        </section>
        ...
        <section className="rounded-xl border border-kampmax-border bg-white p-4" aria-labelledby="timeline-heading">
          ...
          {transaction.orderId && (
            <div className="mt-4">
              <p className="text-sm text-kampmax-text-secondary">Linked to order</p>
              <Link
                href={`/vendor/orders/${transaction.orderId}`}
          ...
          <ul className="mt-4 space-y-4" role="list">
            {transaction.events.map((evt) => (
              <li key={evt.id} className="relative pl-6 border-l border-kampmax-border">
            ...
            {transaction.events.length === 0 && (
              <li className="text-center py-4 text-sm text-kampmax-text-secondary">
